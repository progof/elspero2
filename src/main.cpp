...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Elspero: starting difficulty is 1 / 2^12
...
{
    // Elspero: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Elspero: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Elspero: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Elspero: 5 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Elspero: Special short-term limits to avoid 10,000 BDB lock limit:
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Elspero: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ElsperoMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ElsperoMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ElsperoMiner(CWallet *pwallet)
{
    printf("ElsperoMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("elspero-miner");
    ...
    try { loop {
        ...
        printf("Running ElsperoMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ElsperoMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ElsperoMiner, pwallet));
}
